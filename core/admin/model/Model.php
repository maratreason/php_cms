...
class Model extends BaseModel
{
    ...
    /**
     * Обновление позиции вывода
     *
     * @param string $table - таблица
     * @param mixed $row - по умолчанию menu_position
     * @param mixed $where - если id пришел
     * @param int $end_pos - позиция на которую вставится текущая запись
     * @param array $update_rows - массив, чтобы мы могли сортировать относительно, например, parent_id
     * @return void
     */
    public function updateMenuPosition($table, $row, $where, $end_pos, $update_rows = [])
    {
        if ($update_rows && isset($update_rows['where'])) {
            ...
            if ($where) {
                // получаем позицию, на которой был элемент
                $start_pos = $this->get($table, [
                    'fields' => [$row],
                    'where' => $where
                ])[0][$row]; // возвращем число
        ...
        // Теперь работаем относительно сортировки
